...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const calculate = async () => {
        ...
        setProductDataToStorage(calories, fats, protein, selectedProduct.name, quantity);
        changeValueOfDailyProgress(calories, protein, fats);
        clearInputs();
    };
    const addItemToHistory = (productName, protein, calories, fats, quantity) => {
        const existingItem = history?.querySelector(`[data-product="${productName}"]`);
        const html = `
		<p>
	...
	`;
        if (existingItem) {
            const quantityElement = existingItem.querySelector(".quant");
            const proteinElement = existingItem.querySelector(".history-item-protein");
            const caloriesElement = existingItem.querySelector(".history-item-calories");
            const fatsElement = existingItem.querySelector(".history-item-fats");
